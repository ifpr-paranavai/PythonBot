...
def get_specific_response(question, system_message=""):
    # Mapeamento de perguntas para respostas específicas
    question_to_response = {
       ...
       "Para qual dia vocês tem disponibilidade?": "Para amanhã se desejar, será para uma ou mais pessoas?",
    ...
    # Verifique se a pergunta corresponde a alguma regra específica
    if question in question_to_response:
        return question_to_response[question]
    
    # Se a pergunta não corresponder a nenhuma regra específica, use o modelo ChatGPT
    messages = [
...
def main():
    system_message = "Bem-vindo ao serviço de reservas do Hotel Bellagio! Estou aqui para ajudá-lo com informações sobre o hotel e reservas. Como posso ajudar você hoje?"
    ...
    while True:
        ...
        response = get_specific_response(user_input, system_message=system_message)
